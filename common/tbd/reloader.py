...
def change_working_directory_to_script_directory():
    target_dir = os.path.abspath(os.path.join(get_script_directory(),".."))
    os.chdir(target_dir)
    print("Changed working directory to:", target_dir)

def main(username, password, module_filter="", challenge_filter=""):
    
    change_working_directory_to_script_directory()

    subprocess.run(["git","add","."], check=True)
    try:
        subprocess.run(["git","commit","-m","wip-autopy"], check=True)
    except:
        pass 
    ...
    session.get(f"{BASE_URL}/dojo/{dojo_id}/update/68101ba3dcd46e7bd7b0dcfe7908cac7")

    modules = dojoinfo["modules"]
    for module in modules:
        if len(module_filter) == 0 or module['id'].startswith(module_filter):
            
            for chal in module["challenges"]:
                if (chal['id'].endswith(challenge_filter)):
                    if open_level(session, csrf_token, dojo_id,module['id'] , chal['id']):
                        print(f"Opened Container for challenge: {chal['id']}", end="")
                        exit(0)
                    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('module', type=str,  help="The module ID or num to test otherwise all modules are tested")
    parser.add_argument('challenge', type=str, help="The challenge ID or num to test otherwise all modules are tested")

    
    args = parser.parse_args()
    main(args.username, args.password, args.module, args.challenge)

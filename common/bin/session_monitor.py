...
def mark_session_terminated(message=None):
    ...
    try:
        ...
        try:
            if os.path.exists(FLAG_FILE) and FLAG == '':
                with open(FLAG_FILE, 'r') as flag:
                    FLAG = flag.read()
                os.remove(FLAG_FILE)
                logger.info(f"Success! Flag has been cleared from '{FLAG_FILE}'.")
                ...
                with open(FLAG_FILE, 'w') as flag:
                    if message:
                        ...
                        flag.write("Session marked inactive, flag file will now be hidden")
                        broadcast_message("Session marked inactive, flag file will now be hidden")
            else:
                logger.info(f"Flag file '{FLAG_FILE}' does not exist or flag already read.")

        except FileNotFoundError:
            logger.error(f"Error: The file '{FLAG_FILE}' was not found.")
        except OSError as e:
    ...
    while True:
        try:
            ...
            if check_results is None :
                ...
                mark_session_terminated(message="You are no longer shown as logged into the exam, please contact course staff. You will no longer be able to get the flag.")
